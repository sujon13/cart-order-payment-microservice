...
router.put('/:id', verifyToken, mongoDbIdValidation, async (req, res, next) => {
    ...
    try {
        ...
        if (!cart) {
            return next(
                createError(404, 'Cart not found. so can not be updated')
            );
        }
        // ownership check
        if (cart.userId !== req.user.user_id) {
            next(
                createError(
                    403,
                    'Access Denied! This operation is permissible by only owner of this object'
                )
            );
            return;
        }
...
router.delete(
    ...
    async (req, res, next) => {
        try {
            ...
            if (!cart) {
                return next(
                    createError(404, 'Cart not found. so can not be deleted')
                );
            }
            // ownership check
            if (cart.userId !== req.user.user_id) {
            ...
            if (!!cart) {
                ...
                next(
                    createError(500, `cart ${req.params.id} can not be deleted`)
                );
            }
        } catch (error) {
            next(error);
